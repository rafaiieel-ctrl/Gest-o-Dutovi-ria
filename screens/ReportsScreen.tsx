...
export const ReportsScreen: React.FC<ReportsScreenProps> = ({
    ...
    const handleGenerateReport = () => {
        const allData = [
            ...
            ...dutoviarioSchedules.map(s => ({ ...s, modal: 'dutoviario', product: s.produto, normalizedClient: s.cliente_final, normalizedOrders: [s.pedido_remessa_armazenagem, s.pedido_venda_cliente].filter(Boolean) as string[], normalizedOrigin: s.base_origem, normalizedDestination: s.base_destino })),
        ...
        const mappedData: ReportRow[] = filtered.map(item => {
            ...
            return {
                ...
                tipo_operacao: anyItem.tipo_operacao ? (
                    anyItem.tipo_operacao === 'venda_tanque' ? 'Venda em Tanque' : 
                    anyItem.tipo_operacao === 'transferencia' ? 'Transferência' :
                    anyItem.tipo_operacao === 'transferencia_destino' ? 'Transferência de Destino' :
                    anyItem.tipo_operacao === 'transferencia_propriedade' ? 'Transferência de Propriedade' : '—'
                ) : '—',
