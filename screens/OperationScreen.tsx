...
export const OperationScreen: React.FC<OperationScreenProps> = (props) => {
    ...
    const sidebarTabs = [
        ...
        },
        {
            ...
            content: <AIAnalyst result={props.analysisResult} isAnalyzing={props.isAnalyzing} onAnalyze={props.onAIAnalysis} />
        }
    ];

    return (
        <div className="space-y-6">
            <Breadcrumb items={[{ label: 'Hub de Operações', onClick: props.onBack }, { label: 'Operação' }]} />
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Operação {props.operationDetails.id}</h1>
                    <div className="flex items-center gap-2 mt-1">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusConfig[props.operationDetails.status].className}`}>
                            {statusConfig[props.operationDetails.status].text}
                        </span>
                        <span className="text-sm text-muted-foreground capitalize">{props.operationDetails.modal} • {props.operationDetails.type}</span>
                    </div>
                </div>
                <MainActions onAddTank={props.onAddTank} onCalcAll={props.onCalcAll} onExport={props.onExport} onReport={props.onReport} isVesselOperation={!!props.activeSchedule} />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <OperationDetailsForm details={props.operationDetails} setDetails={props.setOperationDetails} vessels={props.vessels} />
                    
                    {props.activeSchedule && props.activeOperationType === 'loading' && (
                        <RealizadoCard details={props.operationDetails} onUpdate={(f, v) => props.setOperationDetails({ ...props.operationDetails, [f]: v })} schedule={props.activeSchedule} tanks={props.tanks} />
                    )}
                    
                    {props.activeSchedule && props.activeOperationType === 'unloading' && (
                        <DischargeSummaryCard schedule={props.activeSchedule} tanks={props.tanks} />
                    )}

                    <TankList tanks={props.tanks} onAdd={props.onAddTank} onUpdate={props.onUpdateTank} onDelete={props.onDeleteTank} onDuplicate={props.onDuplicateTank} onCalcAll={props.onCalcAll} />
                </div>
                
                <div className="space-y-6">
                    <SidebarTabs tabs={sidebarTabs} />
                    <OperationActions onSave={props.onSave} onLoad={props.onLoad} onNew={props.onNew} onConclude={handleConcludeClick} />
                </div>
            </div>

            <ConfirmationModal isOpen={isConcludeModalOpen} onClose={() => setIsConcludeModalOpen(false)} onConfirm={confirmConclusion} title="Concluir Operação" confirmText="Sim, Concluir">
                <p>Deseja realmente concluir esta operação? Os dados serão arquivados e não poderão mais ser editados nesta tela.</p>
            </ConfirmationModal>
        </div>
    );
};