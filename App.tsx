...
const App: React.FC = () => {
    ...
    const FerroviarioProgrammingScreenWrapper = () => {
        ...
        if (!goal) return <Navigate to="/planning/rail" />;
        return <FerroviarioProgrammingScreen planningGoal={goal} allPlanningGoals={planningGoals} schedule={ferroviarioSchedules} setSchedule={setFerroviarioSchedules} onBackToCenter={() => navigate('/planning/rail')} onBackToHub={() => navigate('/planning')} onStartOperation={handleStartFerroviarioLoading} showToast={showToast} />;
    };

    const FerroviarioLoadingScreenWrapper = () => {
        const { scheduleId } = useParams();
        const currentSchedule = ferroviarioSchedules.find(s => s.id === Number(scheduleId));
        if (!currentSchedule) return <Navigate to="/planning/rail" />;
        return <FerroviarioLoadingScreen schedule={currentSchedule} onUpdateSchedule={(updated) => setFerroviarioSchedules(prev => prev.map(s => s.id === updated.id ? updated : s))} onBack={() => navigate(-1)} showToast={showToast} />;
    };

    const FluvialProgrammingScreenWrapper = ({ type }: { type: 'Balsa' | 'Navio' }) => {
        const { goalId } = useParams();
        const goal = planningGoals.find(g => g.id === Number(goalId));
        if (!goal) return <Navigate to={type === 'Balsa' ? '/planning/fluvial' : '/planning/maritime'} />;
        return <FluvialProgrammingScreen planningGoal={goal} allPlanningGoals={planningGoals} schedule={schedule} setSchedule={setSchedule} vessels={vessels} onBackToCenter={() => navigate(type === 'Balsa' ? '/planning/fluvial' : '/planning/maritime')} onBackToHub={() => navigate('/planning')} onStartOperation={handleStartLoading} onRegisterArrival={handleRegisterArrival} onStartDischarge={handleStartDischarge} onFinalizeTrip={handleFinalizeTrip} onGenerateReport={() => {}} showToast={showToast} />;
    };

    const VesselScreenWrapper = () => {
        const { id } = useParams();
        const vessel = vessels.find(v => v.id === Number(id)) || (id === 'new' ? undefined : null);
        return <VesselScreen vessel={vessel} onSave={handleSaveVessel} onBack={() => navigate('/registration-hub')} />;
    };
        ...
        <div className="min-h-screen md:pl-20">
            ...
            <main className="transition-all duration-300">
                <Routes>
                    ...
                    <Route path="/planning/pipeline" element={<DutoviarioPlanningScreen onBack={() => navigate('/planning')} setSchedules={setDutoviarioSchedules} showToast={showToast} orders={orders} />} />
